...
class MeterStatus(str, enum.Enum):
    ...
    Раньше существовал статус INVALID для счётчиков со сломанным при
    call-home обнаружении серийным номером (буквы вместо цифр) — вкладка
    «Некорректные данные» в UI позволяла исправить его вручную. По
    прямому указанию пользователя (2026-09-14) статус и вкладка убраны:
    такой серийный номер теперь отклоняется сразу при опознании счётчика
    на call-home (см. Gateway CallHomePool._identify, callhome.py) и
    вообще не доходит до Backend."""
...
class Meter(Base):
    ...
    # признак неисправности сбора данных, поэтому отдельная категория,
    # не статус. Проставляется вручную через UI/API либо
