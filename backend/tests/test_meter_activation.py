...
async def test_update_meter_duplicate_serial_number_conflicts(client, db_session):
    ...
    other_meter = Meter(serial_number="900000000058", is_call_home=True, status=MeterStatus.ACTIVE, gateway_id=gateway.id)
    db_session.add(other_meter)
    await db_session.commit()
    await db_session.refresh(other_meter)
    token = await _login(client, "root", "pass1234")

    resp = await client.put(
        f"/api/meters/{other_meter.id}",
